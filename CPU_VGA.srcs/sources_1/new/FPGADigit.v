...
module FPGADigit(LedData,CLK,SEG,AN,RST);
    ...
    counter_3 cot(.clk(CLK_N),.out(count),.halt(1'b0),.RST(RST));//clk counter , T=8
...
module counter_3(clk, out,halt,RST);
	input clk,halt,RST;                    
	output reg [2:0] out;             
    initial out=0;
	always @(posedge clk)  begin
...
module counter_32(clk, out,halt,RST);
	...
	always @(posedge clk)  begin
	    if(RST) out<=0;
        else if(out<32'hffffffff) 
           begin
            if(~halt)  out<=out+32'h00000001;
           end
        else 
           begin
            out<=0;
